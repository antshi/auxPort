...
#' naivePortCalc(covMat)
#'
#'
#' @export naivePortCalc
#'
naivePortCalc <- function(Sigma) {
  n <- dim(Sigma)[1]
...
#' tangPortCalc(covMat, muVec)
#'
#' @export tangPortCalc
#'
tangPortCalc <- function(Sigma, mu, rf = 0) {
  Sigma <- as.matrix(Sigma)
  n <- dim(Sigma)[1]
  mueff <- mu - rf
  ones <- rep.int(1, n)
...
#' Nummerical Portfolio Optimization
#'
...
#' @param results_all a logical. If TRUE, the result inclues the calculated weights and the duals from the optimization. If FALSE, only the weights. Default value is FALSE.
#'
...
#' covMat <- var(example_rets)
#' muVec <- colMeans(example_rets)
#' solverPortCalc(covMat)
#' solverPortCalc(covMat, muVec, porttype="Tang")
#'
#' @export solverPortCalc
#'
solverPortCalc <-
  function(Sigma,
           mu = NULL,
    ...
    #default values penidx1 and penidx2
    if (is.null(penidx1)) {
    ...
    if (porttype == "GMV") {
      ...
      if (is.null(mu)) {
        stop(cat("Please, enter a value for mu, the expected returns vector."))
      }
      ...
      f <- rbind(-mueff, mueff) + lambda1 * rbind(penidx1, penidx1)
...
#' Wrapper Function for Portfolio Optimization
#'
#' Allows the execution of all included optimization functions.
#'
#' @param Sigma a pxp matrix, the covariance matrix of asset returns
#' @param portFunc a function for portfolio optimization
#' @param ... additional arguments to be passed to portFunc
#'
#' @return returns the value of the executed function portFunc
#'
#' @examples
#' data(sp500_rets)
#' example_rets <- sp500_rets[,2:11]
#' covMat <- var(example_rets)
#' PortCalc(covMat, gmvPortCalc)
#'
#' muVec <- colMeans(example_rets)
#' PortCalc(covMat, tangPortCalc, muVec)
#'
#' @export PortCalc
#'
PortCalc <- function(Sigma, portFunc, ...) {
  return(portFunc(Sigma, ...))
}


# Dominant Eigenvalue
eigenPortCalc <- function(Sigma) {
  Sigma <- as.matrix(Sigma)
  p <- dim(Sigma)[2]
...
# Optimal Markowitz
markowitzPortCalc <- function(Sigma,
                              mu,
                              rf = 0,
                              gamma = 2) {
  Sigma <- as.matrix(Sigma)
...
# Optimal Markowitz - unbiased
markowitzUnbiasedPortCalc <-
  function(Sigma,
           mu,
           rf = 0,
           gamma = 2,
    ...
    weights <- as.numeric(unb * markowitzPortCalc(Sigma, mu, rf, gamma))
...
TZMarkowitzPortCalc <-
  function(Sigma,
    ...
    weights_Markowitzunb <-
      markowitzUnbiasedPortCalc(Sigma, mu, rf, gamma, TT)
    n <- dim(Sigma)[1]
    mueff <- mu - rf
    SigmaInv <- solve(Sigma)
    weights_naive <- naivePortCalc(Sigma)
    sharpesquared <- as.numeric(mueff %*% SigmaInv %*% mueff)
    pi1 <-
      as.numeric(
...
TZKZ3fPortCalc <- function(Sigma,
  ...
  TT <- size_sample
  Sigma <- as.matrix(Sigma)
  weights_KZ3f <- KZ3fPortCalc(Sigma, mu, rf, gamma, TT)
  n <- dim(Sigma)[1]
  mueff <- mu - rf
  SigmaInv <- solve(Sigma)
  weights_naive <- naivePortCalc(n)
  ones <- rep.int(1, n)
  mug <-
    as.numeric(mueff %*% SigmaInv %*% ones) / as.numeric(ones %*% SigmaInv %*%
                                                           ones)
...
#' @export effPortCalc
effPortCalc <- function(Sigma, mu, mugrid) {
  n <- dim(Sigma)[1]
  ones <- rep.int(1, n)
  SigmaInv <- solve(Sigma)
...
# Efficient Frontier
effSDCalc <- function(Sigma, mu, mugrid) {
  n <- dim(Sigma)[1]
  ones <- rep.int(1, n)
  SigmaInv <- solve(Sigma)
  A <- as.numeric(ones %*% SigmaInv %*% mu)
  B <- as.numeric(mu %*% SigmaInv %*% mu)
  C <- as.numeric(ones %*% SigmaInv %*% ones)
  sd <- as.numeric(sqrt(C / (C * B - A ^ 2) * (mugrid - A / C) ^ 2 + 1 /
                          C))
...
#' @export cmlPortCalc
cmlPortCalc <- function(Sigma, mu, rf = 0, mugrid) {
  mueff <- mu - rf
  weights_tang <- tangPortCalc(Sigma, mu, rf)
  mu_tang <- weights_tang %*% mueff

  weights <-
    as.numeric(as.matrix(mugrid / mu_tang) %*% t(weights_tang))

  return(weights)
}

cmlSDCalc <- function(Sigma, mu, rf = 0, mugrid) {
  Sigma <- as.matrix(Sigma)
  weights_tang <- tangPortCalc(Sigma, mu, rf)
  mueff <- mu - rf
  mu_tang <- weights_tang %*% mueff
  sd_tang <- t(weights_tang) %*% Sigma %*% weights_tang
  cml.b <- sd_tang / (mu_tang - rf)
  cml.a <- -rf * cml.b
  sd <- as.numeric(cml.a + cml.b * mugrid)
